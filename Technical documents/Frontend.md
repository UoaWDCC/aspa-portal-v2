...
- frontend
  - src
    - assets
    - components - reusable components should be stored here
    - routes - individual web pages should be stored here
...
- See which users have registered to each event.
...
- Purpose: Displays upcoming events and recently registered users
- Props and state:
  - State:
    - events: Holds all events fetched from backend.
    - selectedFilter: Holds one of three filters: "All", "Upcoming", "Past"
    - filteredEvents: Hold events according to the selectedFilter
- Methods and lifecycle events:
  - Methods:
    - fetchAllEvents(): Fetches all events fetched from backend.
  - Lifecycle Events:
    - useEffect: Fetches all events fetched on component mount.
- Dependencies:
  - External Libraries:
    - axios for making HTTP requests.
    - react for creating React components.
- API Integration
  - Describe how the frontend interacts with the backend API (e.g., Axios, Fetch).
    1. API endpoints: GET /events: Fetches all events.

### EventDetails

- Purpose: Display users registered to a specific and allow admins to update the event.
- Props and state:
  - State:
    - events: Holds all events fetched from backend.
    - matchedEvent: Holds the event that matches the current params.
    - editingEvent: Holds a boolean value that describes whether the admin has requested an update request.
    - name: Holds the name of the event
    - description: Holds the description of the event
    - location: Holds the location of the event
    - price: Holds the price of the event.
    - date: Holds the date of the event.
    - file: Holds file attached to the event.
  - Context:
    - currentUser: User authentication details.
  - Params:
    - eventId: Holds the eventId of the current event(page)
- Methods and lifecycle events:
  - Methods:
    - fetchAllEvents(): Fetches all events fetched from backend.
    - submitEventUpdate(): Send an UPDATE request to backend using new event info.
  - Lifecycle Events:
    - useEffect: Fetches all events fetched and find the event that matches the eventId param on component mount.
- Dependencies:
  - External Libraries:
    - axios for making HTTP requests.
    - react for creating React components.
- API Integration
  - Describe how the frontend interacts with the backend API (e.g., Axios, Fetch).
    1. API endpoints: GET /events: Fetches all events.
    2. API endpoints: UPDATE /events/${eventId}: Update current event

### AdminEvents

- Purpose: Allow admins to create new events.
- Props and state:
  - State:
    - name: Holds the name of the event
    - description: Holds the description of the event
...
- Methods and lifecycle events:
  - Methods:
    - onEventFormSubmit(): Send a POST request to backend using new event info.
- Dependencies:
  - External Libraries:
    - axios for making HTTP requests.
    - react for creating React components.
- API Integration
  - Describe how the frontend interacts with the backend API (e.g., Axios, Fetch).
    1. API endpoints: GET /events: POST new event.
